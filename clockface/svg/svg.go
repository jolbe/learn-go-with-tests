...
import (
	...
	cf "github.com/gregor-pifko/learn-go-with-tests/clockface"
)
...
func makeHand(p cf.Point, l float64) cf.Point {
	p = cf.Point{p.X * l, p.Y * l}                       // Scale it to the length of the hand
	p = cf.Point{p.X, -p.Y}                              // Flip it over the X axis to account for the SVG having an origin in the top left hand corner
	p = cf.Point{p.X + clockCentreX, p.Y + clockCentreY} // Translate it to the right position (so that it's coming from an origin of (150,150))
	return p
}
